...
class upGet:
    def __init__(self, infile, innames, outname=None, xsec=1):
        ...
        for name in innames:
            dir = infile[name]
            sumAll += sum(dir["sumweights"].values)
            i=1
            while("testTree;%i" %i in dir):
                ...
                scale = 140000*xsec/sum(dir["sumweights"].values)
...
allTrees.append(upGet(infileAll2, "4top2016", 0.0092))
...
with uproot.recreate("inputTrees2.root") as f:
